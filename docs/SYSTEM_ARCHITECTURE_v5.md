# Quantum Browser v5 — Detailed System Architecture
...
Quantum Browser v5 is structured as a decoupled, multi-process architecture combining a hardened web browser engine frontend, native post-quantum cryptographic primitives, an ad/tracker filtering pipeline, and an external network security daemon managing isolated anonymous circuits.
...
- **Profile Hardening:** Custom launcher (`crates/qualium-browser`) initializes an ephemeral, isolated profile on each launch:
  ...
  - `network.proxy.type = 1` (Manual SOCKS5) bound strictly to `127.0.0.1:<PORT>` (Quantum Daemon).
...
Browser Window
     ...
     ▼
Quantum Daemon (qualium-daemon.exe)
     │
...
| Threat | Description | Quantum v5 Defense |
| :--- | :--- | :--- |
